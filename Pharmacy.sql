...
select sum(amount), count(distinct s.saleid), avg(amount), max(amount), min(amount) from sales s
join payments p on s.paymentid = p.paymentid;

select s.saleid, d.name, dc.categoryname, p.name, pay.paymentmethod, s.saledate from sales s
join saledetails sd on s.saleid = sd.saleid
join drugs d on sd.drugid = d.drugid
join drugcategories dc on d.categoryid = dc.categoryid
join pharmacies p on s.pharmacyid = p.pharmacyid
join payments pay on s.paymentid = pay.paymentid
where s.saledate >= '2024-01-01' and s.saledate <= '2024-05-01'
and pay.paymentmethod = 'Credit Card';
